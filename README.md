<p align="center">
  <img src="./assets/banner.svg" alt="Alpha Soumaré, Business plus Technology builder" width="100%" />
</p>

## Hi, I'm Alpha

I'm a Business and Technology Management student at NYU Tandon who likes turning messy business problems into practical systems. My work sits at the intersection of operations, product, analytics, and strategy.

I am especially interested in technology consulting, strategic consulting, product management, AI-enabled operations, and financial technology.

### What I bring

- Built an applicant tracking workflow that cut screening time by **68%** and reduced time-to-hire from **14 days to 5**
- Supported fleet-technology deployment across **100+ vehicles**, improving dispatch accuracy by **23%**
- Analyzed **$2B+** in commercial real estate portfolios across **10+ U.S. markets**
- Helped coordinate technology evaluation and agile planning across a **30+ student** engineering program
- Teach and support **100+ NYU students** working with C++, Java, and Fusion 360

### Featured projects

| Project | What it demonstrates | Stack |
| --- | --- | --- |
| [Delivery Operations Analytics](./delivery-ops-analytics) | KPI design, regional analysis, executive recommendations | Python, Pandas, Pytest |
| [CRE Underwriting Model](./cre-underwriting-model) | NOI, debt service, DSCR, cash-on-cash return, NPV, and IRR | Python, JSON, Pytest |
| [Business Technology Case Studies](./business-technology-case-studies) | Problem framing, stakeholder alignment, and measurable outcomes | Strategy, Agile, Operations |

### Toolkit

`Python` · `Pandas` · `SQL` · `Tableau` · `Power BI` · `Excel` · `C++` · `Java` · `Agile/Scrum` · `Jira` · `A/B Testing` · `Fusion 360`

### How I think

I care about the full path from idea to adoption: understanding the user, choosing the right technology, measuring the outcome, and making the solution usable for the people who depend on it.

### Let's connect

[Portfolio](https://AlphaSoumare.com) · [LinkedIn](https://www.linkedin.com/in/alphasoumare)

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=soumarealpha2-hub&show_icons=true&hide_border=true&theme=tokyonight" alt="Alpha's GitHub statistics" />
</p>
